...
<aui:form action="<%=danhSachURL %>" method="post" name="frm">
	<aui:layout>
        ...
        <aui:column columnWidth="40" last="true">
        	<aui:select name="xacNhan" label="Xác Nhận">
				<aui:option value=""></aui:option>
				<aui:option value="0">Chưa Xử Lý</aui:option>
				<aui:option value="1">Đã Xử Lý</aui:option>
			</aui:select>
...
<liferay-ui:search-container iteratorURL="<%=interatorURL%>" delta="<%=delta%>"
	...
	<liferay-ui:search-container-row className="com.thuvien.model.Phieumuon"
		...
		<liferay-ui:search-container-column-text name="Actions">
			<liferay-portlet:actionURL var="duyet" name="duyetMuon">
				<liferay-portlet:param name="id" value="<%=String.valueOf(item.getId()) %>"/>
				<liferay-portlet:param name="yes" value="1"/>
			</liferay-portlet:actionURL>
			<liferay-portlet:actionURL var="tuChoi" name="duyetMuon">
				<liferay-portlet:param name="id" value="<%=String.valueOf(item.getId()) %>"/>
				<liferay-portlet:param name="yes" value="0"/>
			</liferay-portlet:actionURL>
			<liferay-portlet:actionURL var="detail" name="detail">
				<liferay-portlet:param name="id" value="<%=String.valueOf(item.getId()) %>"/>
			</liferay-portlet:actionURL>
			<% if(item.getXacNhan()==0){ %>
			<liferay-ui:icon-menu>
		        <liferay-ui:icon iconCssClass="icon-edit" message="Duyệt Cho Mượn" url="<%=duyet %>" />
		        <liferay-ui:icon iconCssClass="icon-trash" message="Từ Chối Cho Mượn" url="<%=tuChoi %>" />
		    </liferay-ui:icon-menu>
		    <% }else{ %>
		    	<liferay-ui:icon-menu>
