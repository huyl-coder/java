...
<form name=frm method=post action="<%=saveURL %>">
	<input type=hidden name="<portlet:namespace/>id" value="<%=vanBanDen.getId()%>">
	<table>
		<tr>
			<td>Số Kí Hiệu:</td>
			<td><input type=text name="<portlet:namespace/>soKiHieu" value="<%=vanBanDen.getSoKiHieu()%>" required></td>
		</tr>
		<tr>
			<td>Sổ Văn Bản:</td>
			<td>
				<select name="<portlet:namespace/>soVanBanId">
					...
					<option <%=(vanBanDen.getSoVanBanId()==soVanBan.get(i).getId())?"selected":""%>  value="<%=soVanBan.get(i).getId()%>"><%=soVanBan.get(i).getTen()%></option>
					<% } %>
				</select>
			</td>
		</tr>
		<tr>
			...
			<td>
				<select name="<portlet:namespace/>loaiVanBanId">
					...
					<option <%=(vanBanDen.getLoaiVanBanId()==loaiVanBan.get(i).getId())?"selected":""%> value="<%=loaiVanBan.get(i).getId()%>"><%=loaiVanBan.get(i).getTen()%></option>
					<% } %>
				</select>
			</td>
		</tr>
		<tr>
			<td>Date:</td>
			<td><input type=date name="<portlet:namespace/>date" value="<%=new SimpleDateFormat("yyyy-MM-dd").format(vanBanDen.getDate())%>" required></td>
		</tr>
		<tr>
			<td>Người Ký:</td>
			<td>
				<select name="<portlet:namespace/>nguoiKiId">
					...
					<option <%=(vanBanDen.getNguoiKiId()==nguoiKi.get(i).getId())?"selected":""%> value="<%=nguoiKi.get(i).getId()%>"><%=nguoiKi.get(i).getTen()%></option>
					<% } %>
				</select>
			</td>
		</tr>
